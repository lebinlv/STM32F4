...
void Write_IIC_Data(unsigned char IIC_Data)
{
   ...
   Write_IIC_Byte(0x40);			//write data
	IIC_Wait_Ack();	
	if(OLED_mode == 0) IIC_Data=IIC_Data;
	else IIC_Data=~IIC_Data;
...
/***********************功能描述： 从左到右循环增加X坐标显示图片****************************/
void OLED_DrawMoveBMP(unsigned char x0, unsigned char y0, unsigned char x1,unsigned char y1,unsigned char BMP[],unsigned char JING[])
{
	unsigned char xs,x,y;
	 unsigned int j=0;
	for(xs=x0;xs<x1;xs=xs+10)
	{
